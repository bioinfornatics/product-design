...
// Dev-only annotation overlay for a Next.js Product Design boilerplate.
// Direct port of product-design's templates/prototype/src/annotate/AnnotationOverlay.jsx
// to a Next.js Client Component (TypeScript, no other behavior changes).
//
...
// /api/goose-annotate route. A Goose session picks these up later (see
// the `product-design-annotate` skill): it screenshots the same
// route/viewport, crops to the recorded bbox, and uses the collected
// component-name guesses to jump straight to the right source file.
//
...
function collectComponentGuesses(x: number, y: number): string[] {
  ...
  const previousPointerEvents: Array<[Element, string]> = [];
  overlayNodes.forEach((node) => {
    const el = node as HTMLElement;
    previousPointerEvents.push([node, el.style.pointerEvents]);
  ...
  for (const [px, py] of points) {
    ...
    while (node && depth < 20) {
      ...
      if (fiberKey) {
        // biome-ignore lint: reaching into React internals is intentional here.
  ...
  previousPointerEvents.forEach(([node, value]) => {
    (node as HTMLElement).style.pointerEvents = value;
  });
...
export function AnnotationOverlay() {
  ...
  const onMouseUp = useCallback(() => {
    ...
    setPending({ bbox: { x, y, width, height }, components });
  }, [active, drag]);
